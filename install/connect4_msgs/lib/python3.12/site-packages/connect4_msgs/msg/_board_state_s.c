...
bool connect4_msgs__msg__board_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("connect4_msgs.msg._board_state", (char *)PyUnicode_1BYTE_DATA(module_attr), 30) == 0);
    assert(strncmp("BoardState", (char *)PyUnicode_1BYTE_DATA(name_attr), 10) == 0);

    Py_DECREF(module_attr);
    Py_DECREF(name_attr);
    Py_DECREF(class_attr);
  }
  connect4_msgs__msg__BoardState * ros_message = _ros_message;
  {  // cells
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      if (!rosidl_runtime_c__int32__Sequence__init(&(ros_message->cells), size)) {
        PyErr_SetString(PyExc_RuntimeError, "unable to create int32__Sequence ros_message");
        PyBuffer_Release(&view);
        Py_DECREF(field);
        return false;
      }
      int32_t * dest = ros_message->cells.data;
...
PyObject * connect4_msgs__msg__board_state__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("connect4_msgs.msg._board_state");
  ...
  {  // cells
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "cells");
    if (!field) {
      return NULL;
    }
    ...
    if (ros_message->cells.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int32_t * src = &(ros_message->cells.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->cells.size * sizeof(int32_t));
      assert(data != NULL);
